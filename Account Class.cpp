...
class account
{
	public:
		...
		void deposit(int amt)
		{
			...
			bal+=amt;
			setbal(bal);
			cout<<"Deposit of Rs. "<<amt<<" is made successfully."<<endl;
		...
		void withdraw(int amt)
		{
			if(bal>=amt)
			{
				...
				bal-=amt;
				setbal(bal);
				cout<<"Withdraw of Rs. "<<amt<<" is made successfully."<<endl;
				cout<<"Balance after Withdrawal is:-\t"<<bal<<endl;
			}
			else
			{
		...
		void Interest(float t,float r)
		{
			float CI,amount;
			cout<<"Compound interest is:- \t"<<endl;
			amount = bal*pow(1+(r/100),t);
			cout<<"Amount after "<<t<<" years = \t"<<amount<<endl;	
			CI = amount - bal ;
			cout<<"CI after "<<t<<" years = \t"<<CI<<endl;	
		}
		int getbal()
		{
			return bal;
